...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument(#here
            "--use_adapter",
            type=str2bool,
            default=False,
            help="Whether to add an adapter network or not",
        )
        ...
        group.add_argument(
            ...
            type=int,
            default=0,
            help="If there is an adapter, set the training step",
        )
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if args.use_adapter: #here
            logging.info(f"Using adapter, training step:{args.training_step}, some parameters will be frozen.")
            if args.training_step == 1:
                ...
                for k, p in model.named_parameters():
                    if k.startswith("encoder.encoders."):
                        ...
                        if layer_id > 11 and layer_id <= 17:
                            logging.info(f"Setting {k}.requires_grad = True")
                            p.requires_grad = True
                        else:
                            p.requires_grad = False
