...
from espnet2.asr.adapter.conformer_adapter import ConformerAdapter
...
adapter_choices = ClassChoices(#here
    "adapter",
    classes=dict(
        conformer=ConformerAdapter,
    ),
    type_check=AbsAdapter,
    default="conformer",
)
...
class ASRTask(AbsTask):
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetASRModel:
        ...
        if getattr(args, "adapter", None) is not None:
            logging.warning("Adding an adapter")
            adapter = ConformerAdapter(**args.adapter_conf)
        else:
            logging.warning("No adapter added")
            adapter = None
