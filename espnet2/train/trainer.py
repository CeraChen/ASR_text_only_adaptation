...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            logging.warning("1. Train and validation for one-epoch")
            ...
            with reporter.observe("train") as sub_reporter:
                all_steps_are_invalid = cls.train_one_epoch(
                    ...
                    optimizers=optimizers,
                    schedulers=schedulers,
                    iterator=train_iter_factory.build_iter(iepoch),   #target domain 
                    subiterator=valid_iter_factory,# .build_iter(iepoch, shuffle=True), #source domain
            ...
            logging.warning("2. LR Scheduler step")
            # 2. LR Scheduler step
            ...
            if not distributed_option.distributed or distributed_option.dist_rank == 0:
                logging.warning("3. Report the results")
                ...
                logging.warning("4. Save/Update the checkpoint")
                ...
                logging.warning("5. Save and log the model and update the link to the best model")
                ...
                logging.warning("6. Remove the model files excluding n-best epoch and latest epoch")
            ...
            logging.warning("7. If any updating haven't happened, stops the training")
            ...
            logging.warning("8. Check early stopping")
    ...
    def train_one_epoch(
        cls,
        model: torch.nn.Module,
        iterator: Iterable[Tuple[List[str], Dict[str, torch.Tensor]]], #3 target domain
        subiterator, # Iterable[Dict[str, torch.Tensor]],
        optimizers: Sequence[torch.optim.Optimizer],
        schedulers: Sequence[Optional[AbsScheduler]],
        scaler: Optional[GradScaler],
        ...
        model.train()
        count_blank = torch.zeros(1000, dtype=torch.long).to("cuda" if ngpu > 0 else "cpu")
        count_none_blank = torch.zeros(1000, dtype=torch.long).to("cuda" if ngpu > 0 else "cpu")
        ...
        import random
        _iterator = iter(subiterator.build_iter(random.randint(0,100), shuffle=True))
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            try:
                _, sub_batch = next(_iterator)
                sub_batch = to_device(sub_batch, "cuda" if ngpu > 0 else "cpu")
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    #retval = model(**batch)
                    #if model.traing_step == 3:




                    sub_batch["target_text"] = batch["text"]
                    sub_batch["target_text_lengths"] = batch["text_lengths"]
                    retval = model(**sub_batch)
                    #else:
                    #    retval = model(**batch)
                    ...
                    if isinstance(retval, dict):
                        ...
                        #count_blank += retval["loss"][1][0]
                        #count_none_blank += retval["loss"][1][1]
                        #retval["loss"] = loss
                    ...
                    #   b. tuple or list type
                        ...
                        #count_blank += stats["loss"][1][0]
                        #count_none_blank += stats["loss"][1][1]
                        #tmp_loss = stats["loss"][0]
                        #stats["loss"] = tmp_loss
            ...
            if distributed:
                ...
                torch.distributed.all_reduce(iterator_stop, ReduceOp.SUM)
        #logging.warning("final count_blank: {}".format(count_blank))
        #logging.warning("final count_none_blank: {}".format(count_none_blank))
