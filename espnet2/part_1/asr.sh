...
num_nodes=8          # The number of nodes.
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.asr_train \
                ...
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --valid_data_path_and_name_and_type dump/raw/train_si284/wav.scp,speech,${_type}  \
                --valid_data_path_and_name_and_type dump/raw/train_si284/text,text,text  \
                --valid_shape_file exp/asr_stats_raw_en_char/train/speech_shape  \
                --valid_shape_file exp/asr_stats_raw_en_char/train/text_shape.char  \
                --resume true  \
                --init_param  \
                --ignore_init_mismatch false  \
                --fold_length 80000  \
                --fold_length 150  \
                --output_dir exp/asr_adapter_3_raw_en_char_sp  \
                --config conf/adapter_3.yaml  \
                --train_data_path_and_name_and_type dump/raw/train_960/wav.scp,speech,kaldi_ark  \
                --train_data_path_and_name_and_type dump/raw/train_960_sp/text,text,text  \
                --train_shape_file exp/asr_adapter_3_raw_en_char_sp/train/speech_shape  \
                --train_shape_file exp/asr_adapter_3_raw_en_char_sp/train/text_shape.char  \
                --ngpu 1  \
                --multiprocessing_distributed True
